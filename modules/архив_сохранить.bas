...
Public Sub save_nk()
        ...
        ' ИСПРАВЛЕНИЕ #6: в оригинале при iCol >= 30 функция выходила молча —
        ' пользователь не знал, что накладная с 30+ позициями не сохранилась в архив.
        ' Теперь показываем предупреждение, чтобы данные не терялись без объяснений.
        If iCol >= 30 Then
            MsgBox "Накладная содержит " & iCol & " позиций." & Chr(10) & _
                   "Архив поддерживает не более 29 позиций в одной накладной." & Chr(10) & _
                   "Накладная НЕ сохранена в архив!", _
                   48, "Архив — превышен лимит позиций"
            Exit Sub
        End If
...
Private Sub copy_to_arh_dann()
        ...
        With ThisWorkbook.Sheets(shNmArh)
            ...
            ' ИСПРАВЛЕНИЕ #7: добавлены точки перед Range — в оригинале Range без точки
            ' обращался к ActiveSheet, а не к архивному листу shNmArh.
            ' Форматирование применялось к случайному активному листу.
...
Private Sub format_arh()
        ...
        With ThisWorkbook.Sheets(shNmArh)
            ' ИСПРАВЛЕНИЕ #7 (продолжение): все Range теперь с точкой — работают
            ' строго с архивным листом, а не с ActiveSheet.
